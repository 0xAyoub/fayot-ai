...
function FlashcardListDetails({ user }) {
  const router = useRouter();
  const { id } = router.query; // Récupérer l'ID de la liste depuis l'URL
  ...
  // Récupérer les données de la liste et des flashcards depuis Supabase
  useEffect(() => {
    const fetchFlashcards = async () => {
      ...
      try {
        setLoading(true);
        console.log("Tentative de récupération du jeu de cartes avec ID:", id);
        
        // 1. Récupérer les détails de la liste
        const { data: listData, error: listError } = await supabase
          .from('flashcard_lists')
          .select('*')
          .eq('id', id);
        
        console.log("Résultat de la requête de liste:", { listData, listError });
        
        // Au lieu d'utiliser .single() directement, vérifier manuellement
        if (listError) {
          console.error("Erreur lors de la récupération du jeu:", listError);
          throw listError;
        }
        
        if (!listData || listData.length === 0) {
          console.error("Aucun jeu de cartes trouvé avec cet identifiant:", id);
          setError("Aucun jeu de cartes trouvé avec cet identifiant.");
          setTimeout(() => {
        ...
        // Vérifier si le jeu appartient à l'utilisateur actuel
        const userJeu = listData.find(item => item.user_id === user.id);
        if (!userJeu) {
          console.error("Ce jeu ne vous appartient pas ou n'existe pas");
          setError("Ce jeu ne vous appartient pas ou n'existe pas");
          setTimeout(() => {
            router.push('/my-cards');
          }, 2000);
          return;
        ...
        console.log("Jeu trouvé:", userJeu);
        setListDetails(userJeu);
        
        // 2. Récupérer les mémocartes associées à cette liste
        const { data: cardsData, error: cardsError } = await supabase
          .from('flashcards')
          .select('*')
          .eq('list_id', id)  // Utiliser l'ID de la liste de cartes
          .order('created_at', { ascending: true });
        
        console.log("Cartes trouvées:", { cardsData, cardsError });
        ...
        console.error('Erreur lors de la récupération des mémocartes:', err);
        setError(err.message || "Une erreur est survenue");
        // Après un délai, rediriger vers la liste des cartes
        setTimeout(() => {
          router.push('/my-cards');
        }, 3000);
      } finally {
      ...
      <div className='flex justify-between items-center bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 transition-all duration-300 py-2 px-3 mx-2 mt-2 mb-1'>
        <div className="flex items-center">
          ...
          <h1 className="text-lg font-bold text-[#25a1e1] flex items-center">
            <span className="mr-2">{listDetails?.title || 'Mémo cartes'}</span>
          </h1>
      ...
      <div className="flex-grow flex flex-col mx-2">
        {loading ? (
          <div className="bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 flex-grow flex flex-col items-center justify-center p-4">
            ...
            <p className="text-gray-700 font-semibold">Chargement des mémo cartes...</p>
          </div>
        ) : error ? (
          <div className="bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 flex-grow flex flex-col items-center justify-center p-4">
          ...
          <div className="bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 flex-grow flex flex-col items-center justify-center p-4">
            <p className="text-gray-700 font-semibold">Aucune mémo carte trouvée.</p>
          </div>
        ) : (
          <>
            {showDownload ? (
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center">
          ...
          <div className="ml-4 bg-[#68ccff]/10 rounded-full px-3 py-1 flex items-center">
            <span className="text-xs font-medium text-[#106996]">{memoCards.length} cartes</span>
          </div>
        </div>
      ...
      <div className="flex flex-1 gap-4 h-[calc(100%-3rem)] overflow-hidden">
        ...
        <div className="flex-1 flex flex-col rounded-2xl">
          {loading ? (
            <div className="bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 flex-grow flex flex-col items-center justify-center p-4">
              <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-[#25a1e1] mb-3"></div>
              <p className="text-gray-700 font-medium">Chargement des mémo cartes...</p>
            </div>
          ) : error ? (
            <div className="bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 flex-grow flex flex-col items-center justify-center p-4">
            ...
            <div className="bg-[#ebebd7] rounded-2xl shadow-md border border-[#68ccff]/30 flex-grow flex flex-col items-center justify-center p-4">
              <p className="text-gray-700 font-medium">Aucune mémo carte trouvée.</p>
            </div>
          ) : (
...
export default withAuth(FlashcardListDetails);
